...
class SolidBrowser extends LitElement {
  render() {
    const { folders, files, historique, items } = this;
    //  const { _test } = this;
    return html`
  ...
  items = ${this.items.map((i) => `item: ${i} <br>`)}
  ...
  async updatePublicFolder(folder){
    ...
    await app._fetcher.load(folder).then(response => {
      ...
      let cont = app._store.match(null, app.LDP('contains'), null, null);
      console.log(cont);
      ...
      sizes.forEach(function(size){
        ...
        if (size.object.value == '4096'){
          ...
          app.folders.push(size.subject.value)
          console.log(app.folders)
        }else{
          //console.log()
        }
  ...
  async updatePublicFolder1(folder){
    var app = this;
    if (folder == undefined){
      folder = this._publicFolder;
    }
    ...
    await app._fetcher.load(folder).then(response => {
      ...
      let sizes = app._store.match(folder, app.STAT('size'), null, null);
      console.log(sizes);
